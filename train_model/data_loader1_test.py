...
class ImageFolder1(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        w, h = map(int, self.list[index]['path'].split('_')[-1].split('.')[-2].split('x'))
    ...
    def getlist(self):
        ...
        def gen_datalist(qp,train_or_test):
            self.yuv_path_list=[]
            name_list=glob.glob("G:/bxl/STRANet-master/STRANet-master/gen_dataset/genzhen_"+str(self.cuSize)+'/'+str(qp)+"/*")
            ...
            for i,path in enumerate(self.yuv_path_list):
                ...
                for mode in range(mode_num):
                    for key,splits in cu_pic['prob'][mode].items():
                        ...
                        data_item['path']="G:/bxl/div_crop/0/"+path.split("\\")[-1].split('.')[0]+".yuv"
...
def get_loader(cuSize,batch_size, num_workers=4, mode='train',debug='train'):
