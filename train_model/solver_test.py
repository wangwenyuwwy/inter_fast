...
from data_loader1_test import get_loader
...
# from new_stf import GENet
from network import *
from myModel import AGFB,HeterogeneousBranch,OptimizedHeterogeneousBranchv2,DSC_Net,EnhancedFeatureExtractor
...
def getflops(model):
  ...
  macs, params = get_model_complexity_info(model, inp_shape, as_strings=False, print_per_layer_stat=False,
                                           verbose=True)
  # print('Macs:  ', macs)
  print('Macs:  ', macs / 32 / 32)
  print('Params: ', params)
...
class Solver(object):
    ...
    def load_model(self):
        ...
        for i,module in enumerate(self.cuSize_list):
            my_load_state_dict(module,torch.load(os.path.join(self.model_path, str(self.cuSize)+'/module-%d.pkl' % (i))))
    ...
    def build_model(self):
        ...
        # self.res.append(AGFB(dim=16))
        # if self.cuSize%5==0:
        #     self.subnet=SmartSubnet(out_dim=6)
        # elif self.cuSize==1:
        #     self.subnet=EfficientQPSubnet(out_dim=6,qp_size=16)
        # elif self.cuSize==2:
        #     self.subnet=EfficientQPSubnet(out_dim=6,qp_size=8)
        # elif self.cuSize==3:
        #     self.subnet=EfficientQPSubnet(out_dim=6,qp_size=8)
        # elif self.cuSize==4:
        #     self.subnet=EfficientQPSubnet(out_dim=6,qp_size=12)
        # if self.cuSize%5==0:
        #    self.subnet=HeterogeneousBranch(16,6,4)
        # elif self.cuSize==1:
        #    self.subnet=HeterogeneousBranch(16, 6,16)
        # # elif self.cuSize == 2:
        # #    self.subnet = HeterogeneousBranch(16, 6, 8)
        # elif self.cuSize==2:
        #    self.subnet=HeterogeneousBranch(16, 6,8)
        # elif self.cuSize==3:
        #    self.subnet=HeterogeneousBranch(16, 6,8)
        # elif self.cuSize==4:
        #    self.subnet=HeterogeneousBranch(16, 6,12)
        ...
        # self.res.append(GENet())
        # self.res.append(Edge_Net_bxl())
        # if self.cuSize%5==0:
        ...
        self.cuSize_list=[self.res[0],
                          self.res[1],
                          self.subnet]
        # getflops(self.subnet)
        
        self.optimizer=optim.Adam(chain(self.res[0].parameters(),
                                        self.res[1].parameters(),
    ...
    # def build_model_bxl(self):
    #   self.res = []
    #   if self.cuSize <= 4:
    #     self.res.append(single_conv(ch_in=1, ch_out=8))
    #   else:
    #     self.res.append(single_conv(ch_in=3, ch_out=8))
    #
    #
    #   self.res.append(Edge_Net_bxl())
    #
    #   self.subnet = Sub_Net_bxl_816()
    #
    #
    ...
    def calculate_loss(self,images,pre,total_acc,total_k2_acc, total_remains,gt_remains,thres=0.1):
        # print(images['gt'].min(), images['gt'].max())
    ...
    def validate(self,valid_loader,thres=0.175):
        with torch.no_grad():
    ...
    def train(self):
        ...
        sum_image=150000
        validation_num=2500 #80000
        # sum_image = 39000
        # validation_num = 650  # 80000
        # sum_image=6000
        # validation_num=100 #80000
        total_remains=0
        total_acc=0.
        total_length=0
        epoch_sum_loss=0
