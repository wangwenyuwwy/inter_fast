...
def import_yuv_10bit(seq_path, h, w, tot_frm, yuv_type='420p', start_frm=0, only_y=False):
    ...
    # init
    y_seq = np.zeros((tot_frm, h, w), dtype=float)
    if not only_y:
        u_seq = np.zeros((tot_frm, hh, ww), dtype=float)
        v_seq = np.zeros((tot_frm, hh, ww), dtype=float)
    
    with open(seq_path, 'rb') as fp:
        for i in range(tot_frm):
            fp.seek(int(blk_size * (start_frm + i)) * 1, 0)  # skip frames
