...
def main(file_name, model_path, is_Window, tot_frm):
    ...
    for qp in range(0,4):
        ...
        for k in range(input_batch.shape[0]//batch_size_5+1):
            ...
            pre = subnet[5](pre, torch.tensor(qp).to(device))
            ...
            for idx,item in enumerate(pre):
                ...
                # split_list = [s for s in item]
                split_list = split_list + [s.item() if isinstance(s, torch.Tensor) else s for s in item]
                if sum(split_list)==0:
        ...
        for k in range(input_batch.shape[0]//batch_size_0+1):
            ...
            pre = subnet[0](pre,torch.tensor(qp).to(device))
            pre=torch.nn.functional.softmax(pre[:,:6],dim=1).cpu()
        ...
        for k in range(input_batch.shape[0]//batch_size_1+1):
            ...
            pre = subnet[2](pre, torch.tensor(qp).to(device))
            pre=torch.nn.functional.softmax(pre[:,:6],dim=1).cpu()
            ...
            for idx,item in enumerate(pre):
                ...
                if split_list[4]>0:
                    if cuh==16:
                        ...
                        qp_list_4.append(0)
                if split_list[5]>0:  #16x32-> 16x16, 8x16
                    if cuh==16:
        ...
        for k in range(input_batch.shape[0]//batch_size_2+1):
            ...
            pre = subnet[1](pre, torch.tensor(qp).to(device))
            pre=torch.nn.functional.softmax(pre[:,:6],dim=1).cpu()
        ...
        for k in range(input_batch.shape[0]//batch_size_3+1):
            ...
            pre = subnet[3](pre, torch.tensor(qp).to(device))
            pre=torch.nn.functional.softmax(pre[:,:6],dim=1).cpu()
        ...
        for k in range(input_batch.shape[0]//batch_size_4+1):
            ...
            pre = subnet[4](pre,torch.tensor(qp).to(device))
            pre=torch.nn.functional.softmax(pre[:,:6],dim=1).cpu()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_path', type=str, default="G:/bxl/STRANet-master/STRANet-master/train_model/trained_models/0217/")
    parser.add_argument('--seq_path', type=str, default="F:/FJR/DataSET/HEVC_Test_Sequence/C/")
