...
for video in glob.glob("G:/bxl/div/000_crop/*"):
    setting=0 #choose from 0 to 3
    is_rotate=False #choose True or False
    y,u,v=import_yuv(video,int(video.split("_")[-1].split(".")[0].split("x")[1]), \
                           int(video.split("_")[-1].split(".")[0].split("x")[0]),1)
    if is_rotate:
        unrotated=[y,u,v]
    ...
    if is_rotate:
        yuv_path="G:/bxl/div_crop_400/"+str(setting+4)+"/"+video.split("\\")[-1]
    else:
        yuv_path="G:/bxl/div_crop_400/"+str(setting)+"/"+video.split("\\")[-1]
    # print(video)
