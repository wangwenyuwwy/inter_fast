...
for qp in [37, 32, 27, 22]:
    if not os.path.exists('./ture_'+str(cuSize)):
        os.mkdir('./ture_'+str(cuSize))
    if not os.path.exists('./ture_'+str(cuSize)+'/'+str(qp)):
        os.mkdir('./ture_'+str(cuSize)+'/'+str(qp))
    # print("lllllllllllll")
    for file in glob.glob("G:/bxl/0208_DTASETS/compressed_"+str(qp)+"/0/*"):
        # print("ppppppppppp")
        valid_flag=1
        ...
        if valid_flag==1:
            ...
            with open('./ori_'+str(cuSize)+'/'+str(qp)+'/'+file.split("\\")[-1].split(".")[0]+".json","w") as write_file:
